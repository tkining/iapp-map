...
(function() {
    ...
    var scale = Z.TupleString("scale"),
        translate3d = Z.TupleString("translate3d");
    var endAction = {
        isExecute: false
    };
    ...
    function ScreenSystem(layout) {
        ...
        Z.onDragV(layout, layout, function(event) {
            ...
            function move(targetPos) {
                ...
                var ratio = Math.abs(targetPos) / layoutHeight / 2;
                Z.css(curPage, "transform", [scale(1 - ratio),
                    translate3d(0, targetPos < 0 ? -layoutHeight * ratio + "px" : -layoutHeight * ratio + targetPos + "px", 0)
                ].join(" "));
                curPage.onMove && curPage.onMove();
